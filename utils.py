...
def polarplot(str,con,ano_score_final,str_ano_label, attr_ano_label):
    pass
    worksheet = xlrd.open_workbook(r'C:\Users\17855\Desktop\1234.xls')
    ...
    for sheet_name in sheet_names:
        ...
        for i in range(rows) :
            ...
            score.append(cell)
    con=np.ones(len(con))-con
    adj, features, labels, idx_train, idx_val,idx_test, \
    ano_label, str_ano_label, attr_ano_label,edge_lable = load_mat("cora")
    ...
    for i in range(len(ano_label)):
        if ano_label[i]==0:
            ...
            if str_ano_label[i]==1:
                colors.append('r')
            if attr_ano_label[i]==1:
                colors.append('g')

    theta=[]
    for i in score:
        theta.append(2*np.pi*random.random())

    fig, (ax1,ax2,ax3) = plt.subplots(1,3,subplot_kw={'projection': 'polar','projection': 'polar','projection': 'polar'})
    for index in range(len(ano_label)):
        ax1.scatter(theta[index], score[index], c=colors[index],alpha=0.2)
        ax2.scatter(theta[index], str[index], c=colors[index],alpha=0.2)
        ax3.scatter(theta[index], con[index], c=colors[index],alpha=0.2)

    ax1.set_rlabel_position(False)
    ax1.grid(False)
    ax1.set_title("Score", va='bottom')
    ax1.axis('off') 

    ax2.set_rlabel_position(False)
    ax2.grid(False)
    ax2.set_title("Str", va='bottom')
    ax2.axis('off')

    ax3.set_rlabel_position(False)
    ax3.grid(False)
    ax3.set_title("Con", va='bottom')
    ax3.axis('off')
    plt.savefig(r'.\1.svg',format='svg')
    plt.show()
