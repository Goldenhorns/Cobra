...
'''
最后的输出添加了edge_lable(标记原始图中不存在的边)
'''
...
parser.add_argument('--path', type=str, default=r"C:\Users\17855\Desktop\dataset")#路径名
...
parser.add_argument('--k', type=int, default=50)  #比较距离的点数
...
if __name__ == "__main__":
    ...
    ori_num_edge = np.sum(adj_dense) #原始边数
    num_node = adj_dense.shape[0]    #节点数
    print('Done. \n')
    ...
    for i_ in pbar:
        ...
        for w in graph.neighbors(max_idx):
            neibor2.append(w)
        for e in neibor2:#对于i_检视新的环境
            if e not in neibor1:
                edge_label[max_idx,e]=1
                edge_label[e,max_idx]=1
        for r in neibor1:#对于max_idx检视新的环境
            if r not in neibor2:
                edge_label[i_,r]=1
                edge_label[r,i_]=1                
